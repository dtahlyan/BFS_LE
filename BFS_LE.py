...
#---------------------------------------------------------------------------#
# read graph from txt
G = nx.read_edgelist(r'/work/d/dtahlyan/Data/network_graph.txt',create_using = nx.DiGraph(),delimiter = "," ,nodetype=int, data=(('LINK_ID',float),('TT',float),('Length',float)))

#---------------------------------------------------------------------------#
#read list of LINK_IDs and corresponding lengths (meters)
Link_Length = pd.read_csv(r'/work/d/dtahlyan/Data/Link_Length_new.csv',low_memory=False, dtype={'LINK_ID':int, 'Shape_Length':float})
...
for xx in range (1):
    print str ('OD pair number is') + str(xx)
    # ---------------------------------------------------------------------------#
    exhaust = 0
    # ---------------------------------------------------------------------------#
    ...
    stop = 0
    start = 0
    while (stop - start < 3600) and p < 50 and exhaust == 0:
        ...
        for y in range(len(n)):
            ...
            while namespace['q_%d' % y].size() > 1 and stop - start < 3600:
